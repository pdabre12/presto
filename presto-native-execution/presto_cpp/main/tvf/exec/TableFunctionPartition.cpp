...
void TableFunctionPartition::initNullPositions() {
  ...
  for (int inputChannel : referencedChannels) {
    ...
    if (it != markerChannels_.end()) {
      ...
      nullPositions_[partitionColumn] =
          markerChannelNullPositions[markerInputChannel];
    } else {
...
RowVectorPtr TableFunctionPartition::appendPassThroughColumns(
  ...
  if (isOnlyPassThrough) {
    ...
    // - Index columns (one or more, depending on number of pass-through
    // sources) The outputType_ contains:
    ...
    size_t numProperColumnsInOutput =
  ...
  if (isOnlyPassThrough) {
    ...
    // For functions with pass-through, copy only the proper columns (not index
    // columns) The function output contains: proper columns + index column(s)
    // Count the number of unique index channels (each represents a different
    // pass-through source)
    ...
    size_t numProperColumns =
        functionOutput->children().size() - numIndexColumns;
