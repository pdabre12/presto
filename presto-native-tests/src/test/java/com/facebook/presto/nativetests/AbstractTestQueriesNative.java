...
//        assertQuery("SELECT (SELECT avg(totalprice) FROM orders GROUP BY custkey, orderdate HAVING avg(totalprice) < a) FROM (VALUES 900) t(a)");
    ...
    public void testShowFunctions()
    {
        if (sidecarEnabled) {
            ...
            assertEquals(functions.get("split_part").asList().get(0).getField(1), "varchar(i10)");
            assertEquals(functions.get("split_part").asList().get(0).getField(2), "varchar(i10), varchar(i11), bigint");
